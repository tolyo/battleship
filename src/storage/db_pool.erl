...
-spec start(pool_name(), non_neg_integer(), non_neg_integer(), connection_params()) ->
    {ok, pid()} | {error, term()}.
%% @doc Create a named pool of PostgreSQL connections.
%% Sample usage: `db_pool:start(db_pool, 5, 10, Params).`
...
%% @doc Remove a previously created PostgreSQL connection pool.
%% Sample usage: `db_pool:stop(db_pool).`
stop(PoolName0) ->
    pooler:rm_pool(pool_name_to_atom(PoolName0)).

-spec query(pool_name(), epgsql:sql_query(), [epgsql:bind_param()]) -> query_result().
%% @doc Run a parameterized query using one checked-out pool connection.
%% Sample usage: `db_pool:query(db_pool, "SELECT $1;", [42]).`
